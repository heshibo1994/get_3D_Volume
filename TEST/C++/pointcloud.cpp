...
int main(int argc, char * argv[])
{
    ...
    fs["scale"]>>scale;
    fs["K"]>>K;
    cout<<x_min<<" "<<x_max<<" "<<y_min<<" "<<y_max<<" "<<z_min<<" "<<K<<" "<<scale<<endl;
    ...
    sor.setStddevMulThresh (0.8);                      //设置判断是否为离群点的阀值，用来倍乘标准差，也就是上面的std_mul
   ...
   string world_plypath = string(argv[7]);       //mbaply
    ...
    //没有滤波
    std::cout << " 切割滤波数量: "<< cloud_cut->width * cloud_cut->height<<std::endl;
    pcl::io::savePLYFileBinary(cloud_resize_path,*cloud_resize);


    pcl::PointCloud<pcl::PointXYZRGB>::Ptr world_resize(new pcl::PointCloud<pcl::PointXYZRGB>);
    for (size_t i = 0;i<cloud->points.size();i++){
            ...
            world_resize->push_back(point2);
        
    }
    std::cout << " 切割滤波数量: "<< cloud->width * cloud->height<<std::endl;
    pcl::io::savePLYFileBinary(world_plypath,*world_resize);
